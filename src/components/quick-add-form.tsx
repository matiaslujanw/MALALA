...
export function QuickAddForm({
  ...
  const [state, formAction, isPending] = useActionStateFeedback(
  ...
  return (
    <section className="rounded-[1.75rem] border border-border bg-card p-5">
        ...
        <div className="mb-4 flex gap-2 rounded-xl border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
          ...
          <div>
            ...
            {Object.entries(state.errors ?? {}).map(
              ([key, value]) =>
                key !== "_form" && <p key={key}>{value.join(", ")}</p>,
            )}
          </div>
        </div>
      )}
      ...
      {state?.ok && (
        <div className="mb-4 flex items-center gap-2 rounded-xl border border-sage-200 bg-sage-50 p-3 text-sm text-sage-900">
